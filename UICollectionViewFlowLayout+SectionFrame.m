...
//    NSArray * indexpaths =     [self.collectionView indexPathsForVisibleItems];
//    NSUInteger sectionCount = [self.collectionView numberOfItemsInSection:section];
//    NSIndexPath * minIndexPath = [NSIndexPath indexPathForRow:sectionCount inSection:section];;
//    NSIndexPath * maxIndexPath = [NSIndexPath indexPathForRow:0 inSection:section];
//    //找出当前section 最大最小可见的cell
//    for (NSIndexPath * indexPath in indexpaths) {
//        if (indexPath.section == section) {
//            if (minIndexPath == nil) {
//                minIndexPath = [indexPath copy];
...
//            }
//            if (maxIndexPath == nil) {
//                maxIndexPath = [indexPath copy];
//            }
//            if (maxIndexPath.row < indexPath.row) {
//                maxIndexPath = [indexPath copy];
//            }
//        }
//    }
//    
//    if (!minIndexPath && !maxIndexPath) {
//        return CGRectZero;
//    }
//        
//    UICollectionViewCell * lastcell = [self.collectionView cellForItemAtIndexPath:maxIndexPath];
//    
//    CGRect fristFrame = [self.collectionView cellForItemAtIndexPath:minIndexPath].frame;
//    CGRect lastFrame = [self.collectionView cellForItemAtIndexPath:maxIndexPath].frame;
//    
//    BOOL flag = [lastcell isKindOfClass:NSClassFromString(@"KMChapterUnfoldCell")];
//    
//    CGFloat cellHeight = fristFrame.size.height * (maxIndexPath.row - minIndexPath.row + 1);
//    if (flag) {
//        cellHeight -= fristFrame.size.height;
//    }
//    
//    if (isContainHeader) {
//        CGRect headerFrame = [self layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionHeader atIndexPath:minIndexPath].frame;
//        fristFrame = headerFrame;
//    }
//    
//    if (isContainFooter) {
//        CGRect footerFrame = [self layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionFooter atIndexPath:minIndexPath].frame;
//        lastFrame = footerFrame;
//    }
//    
//    
//    
//    CGFloat width = lastFrame.origin.x + lastFrame.size.width;
//    CGFloat height = cellHeight + (isContainHeader ? fristFrame.size.height : 0) + (isContainFooter ? lastFrame.size.height : 0);
